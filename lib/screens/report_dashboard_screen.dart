...
class _ReportDashboardScreenState extends State<ReportDashboardScreen> {
  ...
  final List<String> _periodOptions = ['Today', 'This Week', 'This Month', 'Yearly'];
  ...
  Future<void> _loadStats() async {
    ...
    try {
      ...
      for (var item in topSelling) {
        ...
        final val = (item['units_sold'] as num?)?.toDouble() ?? 0.0;
        final pct = sumValues > 0 ? (val / sumValues * 100).toStringAsFixed(0) : '0';
        segments.add(
          PieChartSegment(
            label: label,
            value: val,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to load dashboard stats: ${e.toString().replaceAll("Exception: ", "")}')),
      );
    }
  }
                ...
                SliverPadding(
                              ...
                              Row(
                                ...
                                children: [
                                  Container(
                                    ...
                                    child: const Icon(Icons.shopping_bag_rounded, color: Color(0xFFD97706), size: 20),
                                  ),
                                  Container(
                                    padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                                    decoration: BoxDecoration(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        PieChartWidget(
                          ...
                          chartRadius: 180.0,
                          centerTitle: '\$${_totalRevenue > 1000 ? "${(_totalRevenue / 1000).toStringAsFixed(1)}K" : _totalRevenue.toStringAsFixed(0)}',
                            ...
                            child: Column(
                              children: [
                                  ...
                                  ClipRRect(
                                    ...
                                    child: Builder(
                                      builder: (context) {
                                        final double totalUnits = _categorySegments.fold(0.0, (sum, s) => sum + s.value);
                                        return LinearProgressIndicator(
                                          value: totalUnits > 0 ? seg.value / totalUnits : 0.0,
                  ...
                  PrimaryButton(
                    ...
                    onPressed: () {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Downloading Sales Report PDF...'),
